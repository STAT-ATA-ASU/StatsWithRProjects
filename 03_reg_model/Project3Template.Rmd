...
bibliography: ["proj3.bib", "PackagesUsed.bib"]
output: 
...
is accomplished with the `ggplot2` package created by @R-ggplot2. The data is from @rotten and @imdb.  This document is written with R Markdown using the `rmarkdown`, `knitr`, and `bookdown` packages written by @R-rmarkdown, @R-knitr, and @R-bookdown,  respectively.  
...
your data file will be called `gss`. If you use additional sources, you will need to add them to the `proj1.bib` file and cite them in the project. Delete this paragraph before you submit your project. 
...
```{r, MS}
T1 <- xtabs(~title_type + mpaa_rating, data = movies)
knitr::kable(T1, caption = "Something descriptive here", booktabs = TRUE)
```

Table \@ref(tab:MS) is a contingency table of film type and film rating.  Table \@ref(tab:TAB2) shows formatted regression output.
...
```{r, SS, fig.cap = "Another caption you should complete", , fig.width = 7, fig.height = 5}
ggplot(data = movies, aes(x = title_type, fill = mpaa_rating)) + 
